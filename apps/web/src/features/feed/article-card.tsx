...
export function ArticleCard({ article }: { article: Article }) {
  return (
    <article className="card news-card">
      <div className="news-card__meta"><Badge tone="info">{reasonLabels[article.reasonCode]}</Badge><span>{article.source}</span><span><Clock3 size={12} aria-hidden="true" /> 8분</span></div>
