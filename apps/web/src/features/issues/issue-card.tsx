...
export function IssueCard({ issue }: { issue: Issue }) {
  ...
  return (
    <article className="card issue-card">
      ...
      <p>{issue.summary}</p>
      <div className="issue-card__foot"><span>{issue.articleIds.length}개 기사, {issue.sourceCount}개 출처</span><Link href={`/issues/${issue.id}`}>{ready ? "보도 비교하기" : "준비 상태 보기"} →</Link></div>
    </article>
  );
}
