...
export function AdminResourcePage({ config, role }: { config: AdminConfig; role: Role }) {
  ...
  return <><PageHeader eyebrow={config.eyebrow} title={config.title} description={config.description} actions={<><Badge tone="info">role: {role}</Badge><Button variant="secondary"><RefreshCcw size={16} /> 새로고침</Button></>} />
    <div className="admin-toolbar"><div className="admin-toolbar__filters"><div className="search-field"><Search size={14} aria-hidden="true" /><TextField label="검색" className="field--search" placeholder="이름·ID 검색" /></div><Button variant="secondary"><Filter size={16} /> 상태 필터</Button></div></div>
    ...
    <Dialog open={pending !== null} onClose={() => setPending(null)} title={`${pending?.action.label ?? "변경"} 확인`}><p>대상 <strong>{pending?.itemId}</strong>에 변경을 적용합니다. 동일 사용자 동작 동안 아래 Idempotency-Key를 재사용합니다.</p><code style={{ display: "block", overflowWrap: "anywhere" }}>{pending?.key}</code>{conflict && <div style={{ marginTop: "1rem" }}><StatePanel state="conflict" /><Button variant="secondary" style={{ marginTop: ".7rem" }} onClick={() => setConflict(false)}>최신 데이터 불러와 재검토</Button></div>}<TextareaField id="admin-reason" className="field--spaced" label="변경 사유 (필수)" value={reason} onChange={(event) => setReason(event.target.value)} /><div className="form-actions"><Button variant="secondary" onClick={() => setPending(null)}>취소</Button><Button variant={pending?.action.destructive ? "danger" : "primary"} disabled={!reason.trim() || conflict} onClick={confirm}>사유와 함께 실행</Button></div></Dialog>{toast && <Toast message={toast} />}</>;
}
