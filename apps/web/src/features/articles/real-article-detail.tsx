    ...
    <>
      ...
      <div className="article-layout">
        <article className="card article-main">
          ...
          {ready ? <p className="article-analysis-confidence"><DefinitionTooltip {...analysisTerms.confidence} /><strong>{formatConfidence(article.confidence)}</strong></p> : <StatePanel state={article.analysisStatus === "UNTRUSTED" ? "partial" : "processing"} />}
          ...
          {analysis.isPending ? <StatePanel state="loading" /> : analysis.isError ? <StatePanel state="error" onRetry={() => void analysis.refetch()} /> : analysis.data.assessments.assessments.length === 0 ? <StatePanel state="processing" /> : <div className="article-analysis-list">{analysis.data.assessments.assessments.map((assessment, index) => {
            ...
            return <section className="article-analysis-record" key={String(assessment.id ?? index)}><div className="article-analysis-record__head"><Badge tone={evidence.length ? "positive" : "warning"}>{evidence.length ? "공개 근거 있음" : "공개 근거 없음"}</Badge><span>AI 분석 기록 {index + 1}</span></div><h3>{String(assessment.summary ?? `분석 ${index + 1}`)}</h3>{evidence.length === 0 ? <p className="article-analysis-record__limitation">점수는 생성됐지만 공개 가능한 근거 인용이 함께 제공되지 않았습니다. 이 결과를 단독 근거로 해석하지 마세요.</p> : null}<p className="article-analysis-record__meta"><span><DefinitionTooltip {...analysisTerms.confidence} /> {String(assessment.confidence ?? "미측정")}</span><time>{formatPublishedDate(String(assessment.created_at ?? ""))}</time></p><details className="article-analysis-technical"><summary>분석 기술 정보 보기</summary><dl><div><dt>분석 대상 기사 버전</dt><dd>{String(analysis.data?.assessments.article_version_id ?? "확인 중")}</dd></div><div><dt>사용한 AI 모델</dt><dd>{String(assessment.model_alias ?? "별칭 없음")} / {String(assessment.actual_model_id ?? "모델 ID 비공개")}</dd></div><div><dt>분석 기준 버전</dt><dd>{String(assessment.prompt_version ?? "확인 중")}</dd></div></dl></details></section>;
          })}</div>}
