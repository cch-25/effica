...
it("starts at the article being read and only compares ready articles in its issue", () => {
  ...
  expect(screen.getByRole("img", { name: /가로 편향성/ }).querySelectorAll("circle")).toHaveLength(3);
  const choices = within(screen.getByRole("group", { name: "지도 기사 선택" }));
  ...
  expect(screen.getByRole("link", { name: "이 기사 분석 보기 →" })).toHaveAttribute("href", "/articles/article-01");
...
it("keeps a missing sensationalism measurement explicit", () => {
  ...
  expect(screen.getByText(/과장성 미측정/)).toBeVisible();
});
