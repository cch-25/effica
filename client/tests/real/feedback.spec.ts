...
test("guestbook supports anonymous posting, limits, reloads and mobile navigation", async ({ page }) => {
  ...
  await expect(nav.getByRole("link", { name: "방명록", exact: true })).toHaveAttribute("aria-current", "page");
  await expect(page.getByRole("heading", { name: "방명록", exact: true })).toBeVisible();
  ...
  const content = page.getByRole("textbox", { name: "남길 이야기" });
  ...
  await submit.click();
  await expect(page.getByRole("status")).toContainText("방명록에 글을 남겼어요");
  ...
  await expect(mobile.getByRole("link", { name: "방명록" })).toBeVisible();
...
test("failed posting keeps the draft and recovers without duplicate entries", async ({ page }) => {
  ...
  await page.getByRole("textbox", { name: "남길 이야기" }).fill("읽기 편한 화면이 마음에 들어요.");
  ...
  await expect(page.getByRole("textbox", { name: "남길 이야기" })).toHaveValue("읽기 편한 화면이 마음에 들어요.");
  await page.getByRole("button", { name: "글 남기기" }).click();
  await expect(page.getByRole("status")).toContainText("방명록에 글을 남겼어요");
  await page.reload();
