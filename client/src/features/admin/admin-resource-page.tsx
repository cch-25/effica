...
import { ComparisonReview } from "./comparison-review";
...
export function AdminResourcePage({ configKey, role }: { configKey: string; role: Role }) {
  ...
  return <>
    ...
    <div className="admin-toolbar"><div className="admin-toolbar__filters"><div className="search-field"><Search size={14} aria-hidden="true" /><TextField label="검색" className="field--search" placeholder="이름 또는 ID 검색" value={search} onChange={(event) => setSearch(event.target.value)} /></div><Button variant="secondary" disabled><Filter size={16} /> 서버 상태</Button></div></div>
    {query.isPending ? <StatePanel state="loading" /> : query.isError ? <StatePanel state="error" onRetry={() => void query.refetch()} /> : items.length === 0 ? <StatePanel state="empty" /> : <section className="card">{items.map((item) => <article className="admin-item" key={itemId(item)}><div><div className="news-card__meta"><Badge tone={itemStatus(item).includes("FAIL") || itemStatus(item) === "BLOCKED" ? "danger" : itemStatus(item).includes("ACTIVE") || itemStatus(item).includes("SUCCESS") ? "positive" : "warning"}>{itemStatus(item)}</Badge><span>{itemId(item)}</span></div><h3>{itemTitle(item)}</h3></div><div className="admin-item__meta">{primitiveMetadata(item).map(([label, value]) => <div key={label}><small>{label}</small><strong>{String(value)}</strong></div>)}</div><div className="admin-item__actions">{configKey === "issues" && <ComparisonReview issueId={itemId(item)} title={itemTitle(item)} canApprove={canPerform(role, "publish")} />}{config.actions.map((action) => <Button key={action.label} variant={action.destructive ? "danger" : "secondary"} disabled={!canPerform(role, action.level)} title={!canPerform(role, action.level) ? `${action.level} 권한이 필요합니다` : undefined} onClick={() => open(action, item)}>{!canPerform(role, action.level) && <LockKeyhole size={14} />}{action.label}</Button>)}{config.actions.length === 0 && <Button variant="ghost" aria-label={`${itemTitle(item)} 상세`} disabled><MoreHorizontal size={17} /></Button>}</div></article>)}</section>}
    <Dialog open={pending !== null} onClose={() => setPending(null)} title={`${pending?.action.label ?? "변경"} 확인`}><p>대상 <strong>{pending ? itemId(pending.item) : ""}</strong>에 서버 변경을 적용합니다.</p><code style={{ display: "block", overflowX: "auto" }}>{pending?.key}</code>{pending?.action.needsValues && <TextareaField id="admin-values" className="field--spaced" label="변경 값 (JSON)" value={valuesText} onChange={(event) => setValuesText(event.target.value)} />}<TextareaField id="admin-reason" className="field--spaced" label="변경 사유 (필수)" value={reason} onChange={(event) => setReason(event.target.value)} />{conflict && <StatePanel state="conflict" onRetry={() => void query.refetch()} />}{error && <p role="alert" style={{ color: "var(--danger)" }}>{error}</p>}<div className="form-actions"><Button variant="secondary" onClick={() => setPending(null)}>취소</Button><Button variant={pending?.action.destructive ? "danger" : "primary"} disabled={!reason.trim() || busy} onClick={confirm}>{busy ? "실행 중…" : "사유와 함께 실행"}</Button></div></Dialog>
