...
export function PrivacyActions() {
  ...
  return <>
    <section className="card card--padded">
      <div className="privacy-action"><div><strong>정치 민감정보 별도 동의</strong><p>{sensitive ? (sensitive.granted ? "동의함" : "철회됨") : "상태 확인 중"}</p></div><Button variant="secondary" disabled={busy || !sensitive?.granted} onClick={() => setDialog("withdraw")}>동의 철회</Button></div>
    ...
    <Dialog open={dialog !== null} onClose={() => setDialog(null)} title={dialog === "delete" ? "계정을 삭제할까요?" : "별도 동의를 철회할까요?"}>
      {dialog === "delete" ? <><p>삭제 작업을 시작하려면 <strong>DELETE MY ACCOUNT</strong>를 입력해 주세요.</p><TextField id="delete-confirmation" label="확인 문자열" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></> : <p>철회 즉시 행동 프로필과 개인화가 중지됩니다. 이슈와 기사는 계속 열람할 수 있습니다.</p>}
