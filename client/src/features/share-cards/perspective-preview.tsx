...
export function PerspectivePreview({ displayName, template, compact = false, snapshot, publicView = false, returnTo = "/share/new" }: {
  displayName: string;
  ...
  return <section className={`share-preview consumption-preview${template ? ` share-preview--${template}` : ""}${compact ? " consumption-preview--compact" : ""}`} aria-label={title}>
    ...
    {!snapshot ? <p role="status">뉴스 소비 기록을 확인하지 못했습니다. 잠시 후 다시 불러와 주세요.</p> : <>
      <div className="axis" aria-label={`뉴스 소비 다양성 ${snapshot.diversity_score}/100`}>
        ...
        <div className="axis__labels diversity-descriptions"><span>비슷한 관점 위주</span><span>다양한 관점을 고르게</span></div>
      ...
      <div className="consumption-ideology"><h3>내 설문 결과</h3><IdeologyGraph ideology={snapshot.ideology} /></div>
      {!publicView && <ButtonLink variant="secondary" href={`/onboarding/questionnaire?returnTo=${encodeURIComponent(returnTo)}`}>{snapshot.ideology.completed ? "정치 이념 검사 다시 하기" : "정치 이념 검사 하러 가기"}</ButtonLink>}
    </>}
  </section>;
}
