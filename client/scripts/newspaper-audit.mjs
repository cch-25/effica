...
const results = recheck ? JSON.parse(await fs.readFile(path.join(output, 'audit.json'), 'utf8')) : [];
try {
  for (const [device, viewport] of [['desktop', { width: 1440, height: 1000 }], ['mobile', { width: 390, height: 844 }], ['narrow', { width: 320, height: 740 }], ['tablet', { width: 768, height: 1024 }]]) {
    ...
    for (const route of routes.filter(route => !recheck || recheck.includes(route))) {
      errors = [];
      ...
      const metrics = await page.evaluate(() => {
        ...
        return { overflow: document.documentElement.scrollWidth > innerWidth, offenders: overflow, colored, font: getComputedStyle(document.body).fontFamily, headings: [...document.querySelectorAll('h1')].map(el => el.textContent), width: innerWidth };
      });
      const record = { device, route, status: response.status(), errors: [...errors], ...metrics };
...
if (results.some(r => r.overflow || r.errors.length || r.colored.length || (r.status !== 200 && r.route !== '/missing-page'))) process.exitCode = 1;
