...
2026-09-10 18:34 KST 조회에서 아래 10개 출처 모두 승인 상태와 활성 RSS 어댑터를
확인했다. 모든 출처에서 실제 기사 레코드를 확보했다. 조선일보는 수정된 worker의
공식 RSS 수집 작업이 `SUCCEEDED`로 끝나며 기사 2개를 저장하는 것까지 확인했다.
...
이 수치는 해당 시점 DB의 출처별 기사 수이며 AI 분석 완료 수가 아니다. 개별 수집 시도는
원격 응답 지연이나 시간 제한으로 실패할 수 있다. 이투데이와 문화일보는 이전 수집에
성공한 기록이 있으며, 위 조회에서 마지막으로 완료된 시도는 추가 수정 배포 전의 시간
제한 실패였다. 수집 작업의 성공 여부와 공개 분석 준비 상태를 별도로 제공한다.
